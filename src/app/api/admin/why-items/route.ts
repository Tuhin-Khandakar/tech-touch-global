...
const upsertSchema = z.object({
  ...
  body_en:       z.string().max(600).optional().default(''),
  body_bn:       z.string().max(600).optional().default(''),
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    if (data.id) {
      const { id, ...patch } = data
      const { error } = await supabase.from('why_choose_us_items').update(patch).eq('id', id)
      if (error) throw error
    } else {
      const { id: _drop, ...insert } = data
      const { error } = await supabase.from('why_choose_us_items').insert(insert)
      if (error) throw error
    }
